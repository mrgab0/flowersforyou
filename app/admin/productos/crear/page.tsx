      ...
      <div className={`bg-white p-8 rounded-2xl shadow-sm border border-gray-100 transition-all duration-500 ${
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <input name="images" placeholder="URL Imagen 1 (Principal)" className="p-3 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-[#FF97A4]" required />
            <input name="images" placeholder="URL Imagen 2" className="p-3 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-[#FF97A4]" />
            <input name="images" placeholder="URL Imagen 3" className="p-3 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-[#FF97A4]" />
          </div>

          {/* Especificaciones de Flores */}
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Cantidad de Rosas</label>
              <input name="flowerCount" type="number" placeholder="12" className="p-3 border rounded-xl" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Tipo de Bouquet</label>
              <select name="bouquetType" className="p-3 border rounded-xl bg-white">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Adicionales Disponibles</label>
            <div className="grid grid-cols-2 gap-2 border p-4 rounded-xl bg-gray-50">
              {addons.map((addon) => (
                <label key={addon._id} className="flex items-center gap-2 text-sm">
                  <input type="checkbox" name="addons" value={addon._id} />
