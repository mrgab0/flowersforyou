...
export async function POST(req: Request) {
  try {
    ...
    const deliveryOptionsSummary = (deliveryRes?.data && deliveryRes.data.length > 0)
      ...
      : "- Same-Day Delivery across Houston, Pasadena, Pearland, Katy, and metropolitan areas.";

    const whatsappUrl = siteConfig.whatsappUrl || "https://wa.me/16576988586";
    const whatsappPhone = siteConfig.businessPhone || "+1 (800) 555-3569";
    const storeAddress = `${siteConfig.businessAddress || '6705 Fairway Dr'}, ${siteConfig.businessCity || 'Houston, TX 77087'}`;

    // 2. Definir instrucciones de sistema precisas según idioma
    const systemPrompt = isEn
      ? `You are "Sofia", the friendly, elegant, and expert florist at "Flowers For You LLC" in Houston, Texas.
...
- Website Sections & Links:
  ...
  * WhatsApp & Phone: [📲 WhatsApp](${whatsappUrl}) or call ${whatsappPhone}.
  ...
  * About Us & Floral Blog: [About Us & Blog](/nosotros) (our boutique story, flower care guides, and floral tips).
  * Checkout & Payment: [Cart & Checkout](/checkout) (we accept Zelle, Square, Visa, Mastercard, Amex, Discover, and Boutique Pickup).
- Physical Boutique / Pickup:
  * Address: ${storeAddress}.
  * Boutique Pickup is $0.00 (FREE).
- Delivery Logistics:
  * Same-day delivery in Houston, Pasadena, Pearland, Katy, Sugar Land, The Woodlands, and surrounding metropolitan areas.
  * Delivery fee is automatically calculated at [Checkout](/checkout) based on distance in miles from our boutique ($Base + $Per Mile).
  * Delivery Options:
...
- Secciones y Enlaces de la Web:
  ...
  * WhatsApp y Teléfono: [📲 WhatsApp](${whatsappUrl}) o llamar al ${whatsappPhone}.
  ...
  * Carrito y Pago: [Carrito & Checkout](/checkout) (aceptamos Zelle, Square, tarjetas de crédito/débito Visa/Mastercard/Amex y retiro en boutique).
- Boutique Física y Retiro:
  * Dirección: ${storeAddress}.
  * Retiro en Boutique (Pickup) es $0.00 (Gratis).
- Envíos y Delivery:
  * Entregas el mismo día en Houston, Pasadena, Pearland, Katy, Sugar Land, The Woodlands y áreas metropolitanas.
  * La tarifa se calcula automáticamente en el [Checkout](/checkout) según la distancia en millas desde nuestra boutique (Tarifa Base + Millas).
...
9. Mensajes Completos: Completa SIEMPRE todas tus oraciones y pensamientos con su punto final. NUNCA dejes frases a medias o palabras cortadas.`;
    ...
    if (!apiKey) {
      if (isEn) {
        return NextResponse.json({
          text: `🌸 Hello! I'm **Sofia** at *Flowers For You* (Houston, TX). What special occasion are you looking for flowers for today? ✨`
        });
      }
      return NextResponse.json({
        text: `🌸 ¡Hola! Soy **Sofia** de *Flowers For You* en Houston. ¿Para qué ocasión especial estás buscando flores hoy? ✨`
      });
    }
    ...
    if (!aiResponseText) {
      if (isEn) {
        return NextResponse.json({
          text: `🌸 Hello! I'd be happy to assist you. You can browse all our arrangements in the [Flower Catalog](/productos) or reach out directly on [📲 WhatsApp](${whatsappUrl}) to place your order right away. ✨`
        });
      }
      return NextResponse.json({
        text: `🌸 ¡Hola! Con mucho gusto te asesoro. Puedes ver todos nuestros arreglos en el [Catálogo de Flores](/productos) o contactarnos directo por [📲 WhatsApp](${whatsappUrl}) para tomar tu pedido de inmediato. ✨`
      });
    }
    ...
    return NextResponse.json({
      text: "🌸 Con mucho gusto te ayudamos / We're happy to help. Puedes explorar nuestros ramos en el [Catálogo de Flores / Catalog](/productos) o escribirnos directo a [📲 WhatsApp](https://wa.me/16576988586) para atenderte en tiempo real."
    }, { status: 200 });
  }
}
